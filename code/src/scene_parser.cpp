...
void SceneParser::parseLensCamera() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new LensCamera(center, direction, up, width, height, angle_radians, distance, r);
}
...
void SceneParser::parseLights() {
    ...
    while (num_lights > count) {
        ...
        } else if (strcmp(token, "RoundDisk_Light") == 0) {
            lights[count] = parseRoundDisk_Light();
        } else {
...
Light *SceneParser::parseRoundDisk_Light() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    getToken(token);
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new RoundDisk_Light(position, dir, color, r, emission);
}
// ====================================================================
// ====================================================================
...
Plane *SceneParser::parsePlane() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Plane(normal, offset, current_material, v);
}
...
Triangle *SceneParser::parseTriangle() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new Triangle(v0, v1, v2, current_material, v);
}
...
Mesh *SceneParser::parseTriangleMesh() {
    ...
    Mesh *answer = new Mesh(filename, current_material, v);
...
Transform *SceneParser::parseTransform() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new Transform(matrix, object, v);
}
